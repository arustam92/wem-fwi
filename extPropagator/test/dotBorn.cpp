...
int main(int argc, char **argv)
{
	...
	std::shared_ptr<float2DReg> m (new float2DReg(hypVel));
	std::shared_ptr<float3DReg> d (new float3DReg(data->getHyper()));

	{
		dReflect dr(slow);
		std::shared_ptr<float2DReg> m = slow->clone();
		std::shared_ptr<float2DReg> d = m->clone();
...

{
	...
	sc.dotTest();
		...
		// std::shared_ptr<complex2DReg> bg_wfld(new complex2DReg(slow->getHyper()));
		// bg_wfld->random();
		//
		// std::shared_ptr<float2DReg> m(new float2DReg(slow->getHyper()));
		// std::shared_ptr<complex2DReg> d(new complex2DReg(slow->getHyper()));
		// IC ic(bg_wfld);
