...
Born_full::Born_full (std::shared_ptr<complex1DReg> wave, std::shared_ptr<float2DReg> slow, std::shared_ptr<paramObj> par,
...
Acquisition(slow->getHyper(),par,scoord,rcoord) {
	...
	if (param->getBool("illum",false)) {
		c_ptr_adjoint = &Born_full::c_adj_illum;
	}
	else {
		c_ptr_adjoint = &Born_full::c_adj_reg;
	}
...
void Born_full::forward (std::shared_ptr<float2DReg> model, std::shared_ptr<complex3DReg> data, bool add) {

	if(!add) data->scale(0.);

	std::cerr << "Forward complex born ... " << std::endl;
...
void Born_full::c_adj_illum (std::shared_ptr<float2DReg> model, std::shared_ptr<complex3DReg> data, bool add) {

	if(!add) model->scale(0.);

	std::cerr << "Adjoint complex born illumination... " << std::endl;
		auto start = high_resolution_clock::now();
...
void Born_full::c_adj_reg (std::shared_ptr<float2DReg> model, std::shared_ptr<complex3DReg> data, bool add) {
