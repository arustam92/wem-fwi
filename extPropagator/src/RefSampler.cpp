...
RefSampler::RefSampler(std::shared_ptr<float2DReg> slow, int nref) {
			_slow = slow; _nref=nref;
			_nx = _slow->getHyper()->getAxis(1).n;
			_nz = _slow->getHyper()->getAxis(2).n;
			_nw = _slow->getHyper()->getAxis(3).n;

			_slow_ref.resize(boost::extents[_nw][_nz][nref]);
			index.resize(boost::extents[_nw][_nz][nref]);
		...
		for (int iw=0; iw < _nw; iw++) {
			for (int iz=0; iz < _nz; iz++) {
				for (int iref=0; iref<_nref; iref++) {
					...
					_slow_ref[iw][iz][iref] = (*_slow->_mat)[iw][iz][off];
					if (_slow_ref[iw][iz][iref] <= 0) std::cerr << "Negative ref velocity at the layer " << iz << '\n';
				}
			}
	}
}
...
void RefSampler::getIndexes() {
	...
	for (int iw=0; iw < _nw; iw++) {
		for (int iz=0; iz<_nz; iz++) {
			...
			for (int iref=0; iref<_nref; iref++) {
				_slow_ref_slice[iref] = _slow_ref[iz][iref];
			}
