...
if ('pre' in par):
    ...
    if ('double-' in par['pre']['type']):
        ...
        if ('spline' in par['pre']['type']):
            int = Operator.Spline3D(model_pre,model,type=par['pre']['type'],taper=par['pre']['taper'])
    # else:
...
smoothMod = Operator.Taper(model,model,tap=[10],axes=[1],zero=[0],shift=[(0,0)])
smoothMod2 = Operator.Taper(model,model,tap=[10],axes=[1],zero=[0],shift=[(160,110)])
smoothMod = Op.ChainOperator(smoothMod,smoothMod2)
...
if ('data_taper' in par):
    ...
    if ('scale_offset' in par['data_taper']):
        scaleOp = Operator.ScaleOffset(data,data,geom,scale=par['data_taper']['scale_offset'])
...
der = Operator.Derivative(model,model,which=1,order='exact',f0=geom["fmin"],tmax=tmax/2,dt=dt,alpha=1)
# Regularization operators
...
image.zero()
L2Prob = Prblm.ProblemL2Linear(image,data_tap,bornOpSmooth)
...
LBFGSsolver = LinearSolver.LCGsolver(Stop,logger=logger("Log/%s.txt" % log[-1]))
