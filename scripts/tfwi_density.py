...
data = genericIO.defaultIO.getVector(D+"Dat/denConst.H")
######## prepare the input #############
model = genericIO.defaultIO.getVector("Vel/denSlowBg.H")
...
# int = Operator.CosineInterpolation(slow2,slow,points=points[i])
...
smoothOp = Operator.Taper(model,model,tap=[10],axes=[2],zero=[10])
# rect=([3,1,3],[3,1,3])
# repeat=1
# smoothOp = Operator.Smooth(model,model,rect,repeat,mod='all')

bornOp = denExtWEM.extBorn(model,data,wave,parObj)
bornOpSm = Op.ChainOperator(smoothOp,bornOp)
# bornOpSm = Op.ChainOperator(realSmooth,bornOpSm)
wemOp = denExtWEM.extWEM(model,data,parObj,wave)
...
LBFGSsolver.setDefaults(iter_sampling=1,save_obj=True,save_res=True,save_grad=True,
            flush_memory=True,iter_buffer_size=1,save_model=True,prefix="Inv/1800_density_eps_%s" % (eps) )
