...
class Born_refl : public Operator<complex2DReg,complex3DReg>
{
public:
	Born_refl(std::shared_ptr<Operator<complex2DReg,complex2DReg>> refl, std::shared_ptr<Injection>& inj_rec,
	 std::shared_ptr<OneWay>& oneway, std::shared_ptr<complex2DReg>& bg_wfld,std::shared_ptr<complex2DReg>& sc_wfld);

	void forward(std::vector<std::shared_ptr<complex2DReg>> model, std::shared_ptr<complex3DReg> data, bool add);
...
private:
	std::shared_ptr<complex2DReg> dr;
	std::shared_ptr<complex2DReg>& _bg_wfld, _sc_wfld;
	std::shared_ptr<OneWay>& _oneway;
