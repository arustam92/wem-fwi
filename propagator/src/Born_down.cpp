...
Born_down::Born_down(const std::shared_ptr<complex2DReg>& slow, std::shared_ptr<paramObj>& par,
	...
	std::shared_ptr<complex2DReg>& bg_wfld, std::shared_ptr<complex2DReg>& sc_wfld) : _down(down),_up(up),_reflect(reflect),_inj_rec(inj_rec),_sc_wfld(sc_wfld) {

	lin_down = std::make_shared<LinDown>(slow,par,bg_wfld,down);

}

void Born_down::forward(std::shared_ptr<complex2DReg> model, std::shared_ptr<complex3DReg> data, bool add) {
...
void Born_down::adjoint(std::shared_ptr<complex2DReg> model, std::shared_ptr<complex3DReg> data, bool add) {
