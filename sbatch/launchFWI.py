...
import parallelRun
import numpy as np
def runMultiEpsVpExtFWI(parser):
    ...
    for eps in epsilon:
        jobtag = "_vp_eps_%s" % eps
        dic["epsilon"] = eps
        dic['non_linear_solver']['prefix'] = save_as + '_eps_%s' % eps 
        dic['linear_solver']['prefix'] = lin_save_as + '_eps_%s' % eps 
    ...
    print("Starting multi-epsilon FWI with eps = %s"%args.epsilon)
...
if __name__ == "__main__":
    ...
    args = parser.parse_args()
    if args.epsilon: runMultiEpsVpExtFWI(parser)
